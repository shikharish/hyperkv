...
void test_parser() {
	test_case("test parser", {
		...
		expect("parse: lpush 1 2 3",
			   check_cmd(parse("lpush 1 2 3"), LPUSH, (char *[]){"1", "2", "3"}));
		expect("parse: sadd 1 2 3 4",
			   check_cmd(parse("sadd 1 2 3 4"), SADD, (char *[]){"1", "2", "3", "4"}));
