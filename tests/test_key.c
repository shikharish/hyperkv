...
static void test_del(HashTable *ht) {
	test_case("test del", {
		...
		// test argc
		expect("empty del",
			   compare(ht, "del", "-ERR wrong number of arguments (given 0, expected 1+)\r\n"));
	});
	cleanup(ht);
}
...
static void test_exists(HashTable *ht) {
	test_case("test exists", {
		...
		// test argc
		expect("empty exists",
			   compare(ht, "exists", "-ERR wrong number of arguments (given 0, expected 1+)\r\n"));
	});
	cleanup(ht);
}
...
static void test_type(HashTable *ht) {
	test_case("test type", {
		...
		// test argc
		expect("empty type",
			   compare(ht, "type", "-ERR wrong number of arguments (given 0, expected 1)\r\n"));
		expect("type err argc",
			   compare(ht, "type 1 2", "-ERR wrong number of arguments (given 2, expected 1)\r\n"));
	});
	cleanup(ht);
}
